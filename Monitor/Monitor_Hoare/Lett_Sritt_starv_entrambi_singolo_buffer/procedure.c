...
int Leggi(struct LettScritt * ls) {
	...
	if( ls->numero_scrittori > 0 ) { //c'è un if perché è una signal_and_wait
	...
	leave_monitor( &(ls->m) ); // devo uscire dal monitor per consentire agli altri 
	//lettori di mettersi in attesa su una variabile condition, oppure consenitre ad uno scrittore di andare a scrivere.
...
void Scrivi(struct LettScritt * ls, int valore) {
	...
	leave_monitor( &(ls->m) ); // esco dal monitor per dare la possibilkità ad un altro scrittore di fare una wait sulla variabile condition 
